"""GCP実務ナビ - トピック自動収集モジュール

NEWS_SOURCESからRSS + スクレイピングで最新記事を収集し、
Gemini APIでGCP関連のネタだけフィルタリング＆ランク付けする。
"""
import json
import logging
import re
...
class TopicCollector:
    ...
    def _filter_and_rank(self, items: list[dict]) -> list[dict]:
        """Gemini APIでGCP関連のネタをフィルタリング＆ランク付けする"""
...
GCP（Good Clinical Practice）・臨床試験に関連するものだけを抽出し、
...
【評価基準】
- ICH-GCP、臨床試験規制に直接関連する記事: スコア80-100
- モニタリング、査察、同意取得など実務関連の記事: スコア60-79
- 間接的に臨床開発業界に関連する記事: スコア40-59
- 関連性が低い記事: 除外
...
```json
[
  {{"index": 1, "score": 95, "category": "ICH-GCPガイドライン", "blog_title_suggestion": "ブログ記事タイトル案", "keyword": "SEOキーワード"}}
]
```
スコア40未満の記事は含めないでください。"""
        ...
        try:
            ...
            for r in ranked:
                ...
                if 0 <= idx < len(items):
                    ...
                    item["category"] = r.get("category", "GCP最新ニュース")
    ...
    def _simple_filter(self, items: list[dict]) -> list[dict]:
        """簡易キーワードベースのフィルタリング"""
        keywords = [
            "gcp", "ich", "clinical trial", "monitoring",
            "cra", "crc", "informed consent", "inspection",
            "audit", "pmda",
        ]
        result = []
        for item in items:
